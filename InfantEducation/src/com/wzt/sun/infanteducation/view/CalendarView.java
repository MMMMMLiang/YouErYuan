...
//		canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
	...
	public void setList(List<String> list){
		classLists.addAll(list);
	}
	/**
	 * 添加有课日期显示
	 * @throws Exception 
	 */
	private void addHaveClassData(Canvas canvas) throws Exception{
		// 计算日期
		calculateDate();
		// 按下状态，选择状态背景色
		//drawDownOrSelectedBg(canvas);
		...
		for (int i = 0; i < classLists.size(); i++) {
			Date listDate = ConverToDate(classLists.get(i));
			calendar.setTime(listDate);
			String listYearAndMonth = calendar.get(Calendar.YEAR) + ""
					+ calendar.get(Calendar.MONTH);
			if (curYearAndMonth.equals(listYearAndMonth)) {
				int todayNumber = calendar.get(Calendar.DAY_OF_MONTH);
				dayIndex = curStartIndex + todayNumber - 1;
			}
			
			for (int j = 0; j < 42; j++) {
				int classColor = surface.textColor;
				if (isLastMonth(j)) {
					classColor = surface.borderColor;
				} else if (isNextMonth(j)) {
					classColor = surface.borderColor;
				}
				if (dayIndex != -1 && j == dayIndex) {
					classColor = surface.classTextColor;
					drawCellText(canvas, j, date[j] + "", classColor);
				}
			}
		}
